...
export default function EducationAccordionForm({item, index}){
    const {education} = useDatabase();
    const filePickerRef = useRef();
    const {removeItem, updateItem, addInnerItem, removeInnerItem, updateInnerItem} = useLists(education, 'items');
    ...
    return <AccordionItem  key={item.id} >
        <AccordionButton >
            <FormLabel marginBlock={6}>Education: {item.qualification ||'Unknown'} <Button as={'a'} cursor={'pointer'} position={'relative'} bottom={'2px'} marginLeft={2} size={'xs'} colorScheme={'accent'} onClick={() => removeItem(index)}>Remove</Button></FormLabel>
        </AccordionButton>
        <AccordionPanel pb={4}>
            <Flex  flexDirection={'column'} marginBottom={6}>
                ...
                <Grid marginBottom={4} templateColumns='4fr 4fr' gap={6}>
                    <GridItem w='100%' h='10'>
                        <Input type="text" value={item.qualification} placeholder={'Qualification'} onChange={e => updateItem(index, 'qualification', e)}/>
                    </GridItem>
                    <GridItem w='100%' h='10'>
                        <Input type="text" value={item.institute} placeholder={'Institute'} onChange={e => updateItem(index, 'institute', e)}/>
                    </GridItem>
                </Grid>
                <Grid  marginBottom={4} templateColumns='4fr 4fr' gap={6}>
                    <GridItem w='100%' h='10'>
                ...
                <Box marginTop={6} marginBottom={4} style={{border: '1px solid #ececec', borderRadius: '12px'}} padding={4}>
                    <Flex alignItems={'center'} >
                        ...
                        <input accept="application/pdf, image/*" type={'file'} ref={filePickerRef} style={{display: 'none'}} onChange={handleFileChange}/>
                    </Flex>
