...
export default function LanguageAccordionForm({item, index}){
    const {languages} = useDatabase();
    const {removeItem, updateItem} = useLists(languages, 'items');

    return <AccordionItem  key={item.id} >
        <AccordionButton >
            <FormLabel marginBlock={6}>Language: {item.name ||'Unknown'} <Button as={'a'} cursor={'pointer'} position={'relative'} bottom={'2px'} marginLeft={2} size={'xs'} colorScheme={'accent'} onClick={() => removeItem(index)}>Remove</Button></FormLabel>
        </AccordionButton>
        <AccordionPanel pb={4}>
                ...
                <Grid marginBottom={4} templateColumns='4fr 4fr' gap={6}>
                    ...
                    <GridItem w='100%' h='10'>
                        <Select placeholder="Select option" value={item.level} onChange={e => updateItem(index, 'level', e)}>
                            {languageLevel.map((item, index) => <option key={index} value={item}>{item}</option>)}
                        </Select>
                    </GridItem>
                </Grid>
            </Flex>
