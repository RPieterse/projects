...
export default function ProjectsAccordionForm({item, index}){
    const {projects} = useDatabase();
    const {removeItem, updateItem, addInnerItem, removeInnerItem, updateInnerItem} = useLists(projects, 'items');
    ...
    return <AccordionItem  key={item.id} >
        <AccordionButton >
            <FormLabel marginBlock={6}>Project: {item.name ||'Unknown'} <Button as={'a'} cursor={'pointer'} position={'relative'} bottom={'2px'} marginLeft={2} size={'xs'} colorScheme={'accent'} onClick={() => removeItem(index)}>Remove</Button></FormLabel>
        </AccordionButton>
        <AccordionPanel pb={4}>
            <Flex  flexDirection={'column'} marginBottom={6}>
                ...
                <Grid marginBottom={4} templateColumns='4fr 4fr' gap={6}>
                    <GridItem w='100%' h='10'>
                        <Input type="text" value={item.name} placeholder={'Project Name'} onChange={e => updateItem(index, 'name', e)}/>
                    </GridItem>
                    <GridItem w='100%' h='10'>
